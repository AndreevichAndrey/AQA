const sumArr = function (arr: (number | string | boolean | undefined)[]): number {
    return arr.reduce((sum: number, el) => {
        return typeof el === 'number' || typeof el === 'string' ? sum + Number(el) : (console.log(`'${el}' Invalid type of arg !`), sum);
    }, 0);
};
